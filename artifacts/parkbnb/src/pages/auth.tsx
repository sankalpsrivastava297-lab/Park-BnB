...
export default function AuthPage() {
  ...
  const handleMockLogin = (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    // Upsert a demo user so the ID is always valid against the real DB
    registerUser.mutate({
      data: {
        email: "demo@parkbnb.app",
        name: "Demo User",
        role: "driver",
      },
    }, {
      ...
      onError: () => {
        toast({ variant: "destructive", title: "Login failed", description: "Could not connect to server. Is the API running?" });
        setIsLoading(false);
      },
    });
  };
